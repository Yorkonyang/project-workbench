 ...
 * 项目编号前缀（纯函数）——前端单一真源
 *
 * 与后端 `server/projectCodePrefix.js`（CommonJS 镜像）保持同步，
 * 约定同 `src/lib/hierarchy.js` ↔ `server/hierarchy.js`。
 *
 * 前缀语义（已定稿，勿偏离）：
 *  - 前缀 = 登录人姓名拼音大写首字母（杨永刚→YYG、司佳→SJ）；
 *    多音字取 pinyin-pro 默认读音首字母；非字母字符丢弃；
 *    整名无可用字母则回退 email @ 前缀的大写字母；仍无则占位 'U'。
 *  - 同名首字母去重：按组织结构顺序排名，第 1 名不加数字，
 *    第 k 名(k≥2) 前缀 = 首字母 + (k-1)（第 2 名 SJ1、第 3 名 SJ2 …）。
 *
 * 本模块只提供「计算与分配」的纯函数，不写库（写库在服务端 T02）。
 * 五个纯函数：
 *  - deptKeyOf / orderMembersByOrg / assignMissingPrefixes
 *  - pickNextRootCode
 *  - memberInitials
 */
 ...
 */
export function deptKeyOf(member, deptIdSet, nameToId) {
 ...
 */
export function orderMembersByOrg(members, departments) {
  const byId = new Map();
 ...
 */
export function assignMissingPrefixes(members, departments, initialsOf) {
 ...
 */
export function pickNextRootCode(projects, prefix) {
