...
// L1：用 JSX 组件渲染风险图标，避免 dangerouslySetInnerHTML 注入面
function RiskIcons({ risk }) {
  ...
  return (
    <span className="mr-1 inline" style={{ display: 'inline-flex', alignItems: 'center', gap: '3px', fontSize: '12px', lineHeight: 1 }}>
      {hasOverdue && <span style={{ color: starColor }}>★</span>}
      {hasStalled && (
        <span style={{ display: 'inline-block', width: '9px', height: '9px', borderRadius: '50%', background: circleColor }} />
      )}
    </span>
  );
}
